...
const getAllTransactionsfromServer = (id) => {
    return new Promise((resolve, reject) => {
        const sqlQuery = "select transactions.sub_total, products.name, sizes.name_size as size, products.pictures, transactions.delivery_methods_id, transactions.created_at from transactions join products on transactions.products_id = products.id join sizes on products.sizes_id = sizes.id join users  on transactions.users_id = users.id where transactions.users_id = $1 order by created_at asc";
